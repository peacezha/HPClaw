...
import { buildSyncPlan, type SyncOptions } from './syncPlanner';
...
describe('buildSyncPlan', () => {
  ...
  it('produces download for target-only files when target is remote? Actually upload/download depends on source/target perspective', () => {
    // source = local, target = remote
    // Files only in target (remote) but not in source (local) → download (if deleteExtraneous is true, delete)
    // With deleteExtraneous=false, these are skipped
    const source: FileEntry[] = [];
    const target: FileEntry[] = [
      entry({ name: 'remote_only.txt', path: '/remote/remote_only.txt', size: 50, modifiedAt: 500 }),
    ];
    const plan = buildSyncPlan(source, target, defaultOptions);
    // target-only files with deleteExtraneous=false → skip
    expect(plan.summary.skip).toBe(1);
  });
  ...
  it('computes summary counts correctly for mixed scenario', () => {
    ...
    const target: FileEntry[] = [
      entry({ name: 'conflict.txt', path: '/dst/conflict.txt', size: 150, modifiedAt: 2000 }), // different size → conflict
      entry({ name: 'skip_me.txt', path: '/dst/skip_me.txt', size: 300, modifiedAt: 3000 }),  // identical → skip
